...
export default function SEO({ title, description, image = "/thumbnail.jpg", url = typeof location !== "undefined" ? location.href : "", canonical, twitter = { handle: "@cbdesign", site: "@cbdesign" }, }) {
    useEffect(() => {
        ...
        if (twitter.handle)
            setName("twitter:creator", twitter.handle);
        if (twitter.site)
            setName("twitter:site", twitter.site);
        // Canonical
